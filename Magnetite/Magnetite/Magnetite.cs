...
namespace Magnetite
{
	...
	public class magnetite : ConsoleSystem
	{
		...
		static magnetite()
		{
			magnetite.enabled = true;
		}

		public magnetite() : base() { }
		...
		[ConsoleSystem.User]
		[ConsoleSystem.Help("magnetite.login <rcon.password>", "")]
		...
		[ConsoleSystem.Admin]
		[ConsoleSystem.Help("magnetite.reload <optional = plugin name>", "")]
		public static void reload(ConsoleSystem.Arg arg)
		{
			...
			if (PluginLoader.Plugins.ContainsKey(arg.ArgsStr)) {
				...
				arg.ReplyWith("RustAPI reloaded!");
			} else {
