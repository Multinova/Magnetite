...
namespace Magnetite.Patcher
{
	class MainClass
	{
		...
		private static void BootstrapAttachPatch()
		{
			...
			TypeDefinition MagnetiteBootstrap = MagnetiteAssembly.MainModule.GetType("Magnetite.Bootstrap");
		...
		*/
		public static int Main(string[] args)
		{
			...
			if (args.Length > 0)
				interactive = false;
			Console.WriteLine(string.Format("[( Magnetite Patcher v{0} )]", version));
			try
			{
				rustAssembly = AssemblyDefinition.ReadAssembly("Assembly-CSharp.dll");
				MagnetiteAssembly = AssemblyDefinition.ReadAssembly("Magnetite.dll");
			}
			...
			hooksClass = MagnetiteAssembly.MainModule.GetType("Magnetite.Hooks");
